...
2. Run the **RECIPES** framework to structure any request
3. **Manage by asking, not telling.** Treat the AI as the expert and ask what it thinks instead of dictating steps
4. Use the **flip** to make AI interview you instead of guessing at you
5. Apply **six question types** to interrogate your own thinking
6. Build a reusable **expert** by asking questions until a person shows up
7. Turn corrections into **permanent rules** so the same mistake never happens twice
8. Stand up a working setup on your own machine, with your own account, that outlives tonight
...
An open agent runtime on your own laptop, signed in with an account you already have. It reads a folder of files you control, which means your Lenses, your rules, and your corrections load every time instead of getting re-explained every time.

**Why this one**
- **It's free.** No metered API bill that grows while you learn.
- **It uses a subscription you may already pay for**, rather than per-token billing.
- **It's model-agnostic.** OpenAI, Gemini, Claude. Your choice, swap later.
- **It's OS-agnostic.** Mac or Windows.
...
**Take-home:** the second install path, on a cloud box instead of your laptop, is in the handout with the same steps. Same setup, different machine, so it runs when your laptop is closed. Do it this weekend, not tonight.
...
| 0:08 | Module 1, the order, RECIPES off the table fast | Talk |
| 0:16 | **Ex 1.1, Fix the Order** | Hands |
| 0:23 | Module 2, the conversation. Both directions, six types | Talk |
| 0:38 | **Ex 2.1, The Reverse Interview** | Hands |
| 0:50 | Break | |
| 0:55 | Module 3, the brigade, the Lens, the build sheet | Talk |
| 1:07 | **Ex 3.1, Build the Setup** | Hands |
| 1:25 | Module 4, the loop, correcting out loud | Talk |
| 1:33 | **Ex 4.1, Write Rule One** | Hands |
| 1:40 | **Module 5, the setup, install on their own machine** | Hands |
| 1:52 | Close, brigade, tools menu, CTA | Talk |
| 2:00 | End | |

Talk is 59 minutes of 120. **Hands are 56.** If the room is running long, the cut order is: Ex 1.1 first (it's the most replaceable), then trim Module 3's teach, never Ex 3.1 and never Module 5.
